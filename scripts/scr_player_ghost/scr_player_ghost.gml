function scr_player_ghost() //scr_player_ghost
{
    ...
    if (sprite_index != spr_ghostend)
    {
        ...
        {
            instance_create((x + random_range(-25, 25)), (y + random_range(-25, obj_baddiebutton)), obj_cloudeffect)
            ghosteffect = 5
        }
        if (ghosteffect > 0)
            ghosteffect--
        ...
        else
        {
            ...
            if (m != 0)
            {
                ...
                if (mag >= 2.5)
                {
                    ...
                    else if (character == "N" || (!ispeppino))
                    {
                        if (_h && vsp > 0)
                            sprite_index = spr_playerN_ghostdashdownright
                        else if (_h && vsp < 0)
                            sprite_index = spr_playerN_ghostdashupright
                        else if _h
                            sprite_index = spr_playerN_ghostdash
                        else if (vsp > 0)
                            sprite_index = spr_playerN_ghostdashdown
                        else if (vsp < 0)
                            sprite_index = spr_playerN_ghostdashup
                    }
                }
                else
                    sprite_index = spr_ghostidle
