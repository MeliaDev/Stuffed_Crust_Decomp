	...
	switch (_attack.attack)
	{
		...
		case 2:
			...
			if (_attack.direction == 1)
				...
				xx = 862;
			with (instance_create(xx, yy, 171))
			{
				attack = _attack;
		...
		case 4:
			...
			if (_attack.direction == -1)
				xx = _attack.x2;
			with (instance_create(xx, _attack.y, 171))
			{
				attack = _attack;
			...
			break;
		case 5:
			with (instance_create(_attack.x1, _attack.y, 171))
			{
				attack = _attack;
				image_xscale = 1;
			}
			with (instance_create(_attack.x2, _attack.y, 171))
			{
				attack = _attack;
				image_xscale = -1;
			}
	...
	else if (place_meeting(x, y + (sign(vsp) * yy), obj_solid))
	{
		...
		if (superjumpbounce > 0 || superjumpdir == 0)
		{
			...
			if (hitvsp > 0)
			{
				sprite_index = spr_fakepeppino_superjumpstart;
				repeat (10)
					create_debris(x + irandom_range(-25, 25), y + 43, 3226);
				with (instance_create(x, y + 30, 965))
				{
			...
			else if (hitvsp < 0)
			{
				...
				repeat (10)
					create_debris(x + irandom_range(-25, 25), y, 3226);
				with (instance_create(x, y + 20, 965))
				{
					sprite_index = spr_fakepepsplasheffect;
					image_speed = 0.35;
					image_yscale = -1;
	...
	if (sprite_index == spr_fakepeppino_throwhead)
	{
		...
		else
		{
			...
			if (floor(image_index) >= 32 && !instance_exists(headID))
			{
				...
				with (instance_create(x, y - 20, 263))
				{
					other.headID = id;
					thrown = true;
...
function scr_fakepepboss_backbreaker()
{
	...
	if (vsp > 0 && sprite_index == spr_fakepeppino_fall && !taunted)
	{
		...
		instance_create(x, y, 1001);
		with (instance_create(x, y, 130))
		{
