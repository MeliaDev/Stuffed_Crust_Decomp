...
for (var i = i1; i < array_length(input); i++)
{
    ...
    if (controller && i >= 8)
        i2++
    draw_sprite_ext(spr_controlicons, i2, ((obj_screensizer.actual_width / 2) - 60), yy, 1, 1, 0, c, 1)
    if (!controller)
    {
        if (val != -4)
    ...
    else if (val != -4)
    {
        ...
        if (index != -4)
            draw_sprite_ext(spr_tutorialgamepad, index, ((obj_screensizer.actual_width / 2) + 90), (yy - 16), 1, 1, 0, c, 1)
        else
            draw_text_color(((obj_screensizer.actual_width / 2) + 100), yy, lang_get_value("option_press"), c, c, c, c, 1)
    }
    else
        draw_text_color(((obj_screensizer.actual_width / 2) + 100), yy, lang_get_value("option_none"), c, c, c, c, 1)
}
